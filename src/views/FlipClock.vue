...
export default {
  ...
  methods: {
    ...
    init() {
      let now = new Date()
      let nowTimeStr = this.formatDate(new Date(now.getTime()), 'yyyyhhiiss')
    ...
    run() {
      this.timer = setInterval(() => {
        // 获取当前时间
        let now = new Date()
        let nowTimeStr = this.formatDate(new Date(now.getTime() - 1000), 'yyyyhhiiss')
        let nextTimeStr = this.formatDate(now, 'yyyyhhiiss')
        for (let i = 0; i < this.flipObjs.length; i++) {
